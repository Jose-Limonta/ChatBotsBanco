...
public class AccionesAPI {
	...
	public Map<Object, Object> getClienteByClave(String claveIdUser) throws UnirestException {
		...
		return !claveIdUser.isEmpty() ? getOnlyOneMapFromHttpResponse(Constantes.URL_USUARIOS + claveIdUser)
				: new HashMap<>();
	}
	...
	private Map<Object, Object> setAPIAction(Object classAction, String urlAction, String type)
		...
		HttpResponse<String> response = request.headers(headers).body(bodyValue).asString();
	...
	private HttpRequestWithBody getRequest(String type, String urlAction) {
		switch (type) {
		case "post":
			return Unirest.post(urlAction);
		case "put":
			return Unirest.put(urlAction);
		default:
			return Unirest.delete(urlAction);
		}
	}
